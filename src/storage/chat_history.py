...
from storage.paths import chat_history_path

DEFAULT_DB_PATH = chat_history_path()
MAX_THREAD_TITLE_LENGTH = 80
def _now(): return datetime.now(timezone.utc).isoformat()
def _database_path(path=None): return Path(path) if path is not None else chat_history_path()
@contextmanager
def _connect(path=None):
    path = _database_path(path); path.parent.mkdir(parents=True, exist_ok=True); conn=sqlite3.connect(path, timeout=5); conn.execute("PRAGMA foreign_keys=ON")
...
def create_thread(prompt, path=None):
    ...
    return ident
def list_threads(path=None, limit=20):
...
def rename_thread(thread_id, new_title, path=None):
    ...
    return title
def add_message(thread_id, message, path=None):
...
def load_messages(thread_id, path=None):
    initialize_database(path)
    ...
    return result
def delete_thread(thread_id, path=None):
    initialize_database(path)
    with _connect(path) as c:
