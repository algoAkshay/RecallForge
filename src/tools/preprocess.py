...
class CustomDocumentLoader(BaseLoader):
    ...
    async def lazy_load(self) -> Iterator[Document]:
        ...
        """
        file_ = self.file
        retrieved_at = file_.get("retrieved_at") or datetime.now(timezone.utc).isoformat()
        ...
        if "snippet" in file_.keys():
            yield Document(
                ...
                metadata={
                    ...
                    "source": file_["title"],
                    "link": file_["link"],
                    "source_url": file_["link"],
                    "title": file_.get("title"),
                    "document_content_hash": file_.get("document_content_hash"),
